...
var config = require('yargs')
  ...
  .options({
    'listen-port': {
      ...
      describe: 'OAuth request scope to use with "oauth2" method',
      default: process.env.OAP_SCOPE || 'user:info,user:check-access,user:list-projects',
    }, 'transform': {
      // note: would like to be able to specify multiple in env var, but yargs.choices() does not validate input as comma-separated array
...
config.scopes = typeof(config.scope) === 'string' ? config.scope.split(',') : config.scope;
